...
find_usezoning <- function(long_rec, lat_rec){
  longcut <- usezoning_fty[which(usezoning_fty$long <= long_rec + 0.00566 &
                                 usezoning_fty$long >= long_rec -0.00566),]
  ...
  result <- latcut %>% select(id, usz) %>% 
    group_by(usz) %>% unique() %>% summarise(n=n())
  gg <- ggplot(latcut, aes(x=long, y=lat, group=group, col=usz)) + geom_path()
